...
class Tests(TestCase):
    def test_external(self):
        app = dash.Dash(assets_folder='resources_dash/assets', assets_ignore='load_after.+.js')
        app.components = {dcc.__name__}
        app.layout = dcc.Markdown()
        app.scripts.config.serve_locally = False

        with mock.patch('dash.dash.os.stat', return_value=StatMock()):
            resource = app._collect_and_register_resources(
                app.scripts.get_all_scripts(affix=getattr(app, '_res_affix', ''), module_names=app.components))
    ...
    def test_internal(self):
        ...
        self.assertTrue('fake_dcc.min.js.map' in app.registered_paths['dash_core_components'],
